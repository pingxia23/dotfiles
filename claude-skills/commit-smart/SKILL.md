---
name: commit-smart
description: "Deterministic workflow to stage changes, run bzl tests when applicable, commit with hooks, push, and create GitHub draft PRs. Trigger this skill whenever the user asks to commit (for example: 'commit', 'commit this', 'please commit')."
---
...
## Step 6: Create Draft PR (WIP Title)
Intent:
...
4. If `pr_url` exists:
   ...
   - Return the existing URL as final PR output for this step (no update path).
